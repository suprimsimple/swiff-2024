...
const getSshPushCommands = ({
  ...
  const flags = [
    ...
    `-e "ssh -p ${port}${!isEmpty(sshKeyPath) ? ` -i '${sshKeyPath}'` : ""}"`,
  ];
  ...
  const rsyncCommands = pushFolders
    .map((item) => {
      ...
      const rSyncFrom = `${path.join(pathConfigs.pathApp, localPath)}/`;
...
const getSshDatabase = async ({
  ...
  const downloadTo = `${pathConfigs.pathBackups}/${gzipFileName}`;
        ...
        `${
          String(errorMessage).includes("No such file")
            ? `There was an issue downloading the remote ${colourAttention(
                ...
                "remote .env"
              )}? \n\n${colourAttention(JSON.stringify(remoteEnv, null, 2))}`
            : errorMessage
        }`
      )
    );
